...
- 已根据当前仓库真实状态重写根目录 `README.md`：
  ...
  - 快速开始区分为 `judge` 一键体验和本地开发两条路径
  - 测试账号与启动命令均以当前 `.env.example`、`.env.judge.example`、`scripts/judge-up.sh`、前后端实际入口为依据
  - 本轮只改文档说明，未修改业务代码、数据库 schema 或部署脚本
- 已修复首页 / 登录页顶部重复登录入口问题：
...
- 已对活动管理页右侧栏执行一轮最小宽度修复：
  ...
  - 本轮只修改列宽策略，不调整“新增活动 / 自定义首个票种 / 进阶设置 / 活动加票和状态切换”的内部布局
- 已完成最小校验：
  - `pnpm --filter web typecheck`
- 已将比赛演示文档中的双账号提词部分进一步收缩为“最终动作表”：
  - 只保留“时长 / 账号 / 动作 / 要说的话”四列表格
- 已将整份比赛演示脚本文档进一步压缩为极简版：
  - 删除前置分析与说明段落
  - 仅保留最终动作表
- 已将比赛演示脚本进一步改写为“纯浏览器主视频版”：
  - 主视频限定为同域名前台与 `/admin` 工作台
  - `judge-up / docker-compose / CI / 自动化测试 / verification` 全部降为答辩与附件材料
  - 主视频主线现已收口为：
    - 学术空间闭环
    - 体育设施闭环
    - 活动报名支付闭环
...
- 本轮计划只收口最终答辩前的高收益修补项，重点锁定：
  ...
  - 最小路由懒加载
  - 规则引擎答辩说明
- 本轮未修改业务代码、数据库 schema 或部署脚本，只新增计划文档并同步进度记录
- 已将比赛演示策略正式落库为闭环脚本方案：
  - `docs/architecture/competition-demo-video-script-2026-04-22.md`
- 已更新架构文档索引：
  - `docs/architecture/README.md`
- 已把本轮视频方案的环境风险同步写入方案文档：
  - 当前工作区复跑 `pnpm --filter api test` 时为 `25` 个测试中 `20` 通过、`5` 失败
  - 因此主视频建议展示测试入口、CI 与 verification 证据，而不是现场全量跑测试
