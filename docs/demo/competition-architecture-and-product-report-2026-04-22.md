...
> 本文面向比赛评审，基于本项目截至 `2026-04-22` 的参赛版本真实实现整理。
...
项目背景很明确：校园里的空间、场地和活动都在被频繁使用，但它们的预约规则并不相同。如果只做一个统一表单，很容易出现时间冲突、资源重复占用、热门活动超发、支付与取消状态打架等问题。

本项目的核心目标，是在一个统一平台内同时解决三件事：

- 让学生用同一入口完成预约、报名、查看订单和后续处理
- 让管理员可以统一维护资源、活动和规则
- 让系统在高并发、冲突判断和状态变化下仍然保持结果正确

因此，本项目的重点不是普通的增删改查，而是“资源差异如何建模”“冲突如何拦截”“热点场景如何防超卖”“状态变化如何保持一致”。这也是本项目设计的核心出发点。

# 2. 产品设计

从产品视角看，本项目不是单一功能页，而是一个围绕校园生活场景组织的统一服务平台。
...
学术空间采用连续时间预约，而不是固定整点槽位。原因很简单：自习室、讨论室这类资源，用户天然会按“几点到几点”来思考，而不是按一格一格的时段去拼接。

本项目在学术空间中加入了“前后各 5 分钟缓冲”机制。它的含义是：

- 用户看到并选择的是自己的实际使用时间
- 系统在冲突判断时，会自动把这个时间段向前和向后各扩 5 分钟

这样做的设计目的，是避免上一组刚结束、下一组立刻贴边进入，导致真实使用中出现交接混乱。对用户来说，效果是清楚而直接的：

- 页面可以看到连续时间的占用情况
- 系统会提前拦截贴边冲突，而不是等提交后才产生混乱结果
- 精确边界可解释，例如贴边 4 分钟会判冲突，贴边 5 分钟可以放行

系统底层必须拦截冲突，而不能只靠前端提示。因为在并发提交时，只有数据库层的最终约束才能保证“同一房间同一时段只能成功一单”。这也是本项目不是普通预约表单的关键差异。

## 3.2 体育设施

体育设施采用按 `1` 小时 slot 预约，而不是连续时间段。这是因为体育场馆的运营通常按标准时段开放，整点预约对学生更容易理解，对管理员也更容易排班和统计。
...
“组合资源”的意义在于，它不是简单地把多个场地拼在一起展示，而是要支持“一次下单、一并成功、一并失败”。例如团体训练、联动场地使用，就更适合这种模式。
...
校园活动的难点不在“能不能报名”，而在“热门活动大量用户同时点击报名时，系统是否还能保持正确”。

热门活动天然存在高并发问题。若所有请求都直接进入数据库，系统很容易在短时间内承受大量重复判断和写入，既影响响应速度，也容易让库存控制变得不稳定。
...
这样设计的目的，是让数据库只处理真正可能成功的请求，同时保留最终一致性。
...
本项目将预约与活动报名统一收口到订单中心，而不是让每个业务模块各自维护一套状态。这么做的目的，是让状态变化可追踪、可审计、可补偿。
...
因此，本项目把订单状态机作为统一中枢来设计，而不是把这些变化散落在各个业务模块里。

## 3.5 规则限制

校园场景的业务限制很多，例如信用分、预约时长、用户角色、爽约处罚等。如果这些规则全部写死在业务代码里，短期看似简单，长期会越来越难维护。
...
本项目没有把规则散落成大量 if-else，而是采用“规则配置 + 统一入口 + 类型化执行器”的方式。这样既方便后台管理，也方便后续扩展新规则。

# 4. 架构设计

本项目采用单机容器化架构，重点不是堆复杂基础设施，而是在比赛约束下把核心链路拆清楚、跑稳定。
...
- Redis 用于提速，PostgreSQL 用于保证最终正确性
- Nginx 统一对外入口，便于 judge 环境和正式环境切换
...
平台并没有把所有空间都混成同一种资源，而是把“统一性”和“差异性”分开处理：
...
如果把两类资源强行混成一种模型，短期看似简单，实际上会带来三个问题：
...
这种设计的重点在于：缓存负责快，数据库负责准，队列负责削峰，三者分工清楚。
...
这样做的价值，不只是避免状态错误，更重要的是让系统在边界场景下仍然有清楚、可审计的结果。
...
stateDiagram-v2
    [*] --> CONFIRMED: 学术/体育预约成功
    [*] --> CONFIRMED: 免费活动报名成功
    [*] --> PENDING_CONFIRMATION: 付费活动报名成功
    PENDING_CONFIRMATION --> CONFIRMED: 支付成功
    PENDING_CONFIRMATION --> CANCELLED: 超时取消/主动取消
    CONFIRMED --> CANCELLED: 主动取消
    CONFIRMED --> NO_SHOW: 未签到
```
...
因此，状态机不是展示层概念，而是整个业务正确性的基础。

## 7.2 规则模块思路

本项目的规则系统采用“统一规则入口 + 强类型执行器 registry”的方式，而不是通用脚本解释器。
...
本项目的前端设计目标，不只是“页面能打开”，而是让复杂业务被整理成清楚、稳定、易操作的流程。
...
这样做的目的，不是追求样式复杂，而是保证不同设备下都能顺利完成预约和报名。
...
在学术空间、体育设施、校园活动等关键页面，系统不会只用颜色表达结果，而是通过显式文字说明“当前状态、剩余数量、当前选择、是否可操作”，降低理解成本。
...
这些实践的目的，是让页面不仅“看起来能用”，也能被更稳定地操作和理解。

## 8.4 性能优化

本项目没有把所有页面都塞进一个前端首包，而是按路由做了拆分：
...
这套工程化设计的价值，在于方便评委验收：不是只把代码交上来，而是尽量把“启动、验证、演示”变成一条可重复执行的标准路径。

# 10. 当前完成情况与后续优化

## 10.1 当前版本已完成的核心能力

当前参赛版本已经完成并可演示的核心能力包括：

- 学术空间连续时间预约与 `5` 分钟缓冲冲突控制
- 体育设施按 `1` 小时 slot 预约与组合预约
- 校园活动高并发报名、防超卖和库存恢复
- 统一订单中心与状态日志
- 付费活动待支付主链路与模拟支付闭环
- 超时取消、迟到支付补偿与状态一致性保护
- 爽约转 `NO_SHOW` 后的扣分、画像和预约限制
- 教师工作台的资源、规则、活动、通知与工单模块
...
这意味着本项目不仅有界面，也已经具备从前端到后端、从数据库到异步任务的完整链路展示能力。
...
这些内容更适合作为后续产品化深化方向，而不影响当前参赛版本对核心题目的完整回应。

# 11. 总结

本项目最关键的设计价值，在于没有把校园预约题目简化为普通信息录入，而是围绕校园场景中真正重要的问题展开：不同资源如何建模、冲突如何准确拦截、热门活动如何防超卖、订单状态如何保持一致、规则如何可配置地进入业务链路。

因此，它的“扣题”体现在三个方面：

- 产品上，覆盖了学术空间、体育设施、校园活动三类典型校园服务
- 架构上，围绕高并发、防冲突、一致性和状态机做了针对性设计
- 工程上，具备可部署、可验证、可演示的完整闭环

对于比赛评审而言，这个项目的核心意义不只是“功能齐全”，而是“每一项关键设计都能对应题目中的真实难点，并且已经在参赛版本中落成可验证的实现”。
