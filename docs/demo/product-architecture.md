...
它的重点不是把预约表单做全，而是解决四个问题：资源差异如何建模、冲突如何拦截、热点场景如何防超卖、状态变化如何保持一致。这也是整份架构设计的出发点。

# 2. 产品设计

从用户视角看，平台拆成五个清楚的模块，而不是把所有业务堆在一个页面里。
...
本项目没有把校园预约简化成普通表单系统，而是围绕校园场景里的几个关键难点展开：不同资源如何建模、冲突如何准确拦截、热门活动如何防超卖、订单状态如何保持一致、规则如何进入真实业务链路。
