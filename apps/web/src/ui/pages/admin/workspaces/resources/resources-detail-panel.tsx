        ...
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            ...
            <p className="mt-2 text-sm text-slate">
              {localeText(
                locale,
                "这里只维护该资源下的可预约单元，确保学生端能形成明确的预约对象。",
                "Maintain the bookable units under this resource so the student side has clear booking targets."
              )}
            </p>
          </div>
          <StatusPill tone={selectedResource.units.length === 0 ? "danger" : "success"}>
            {selectedResource.units.length === 0
              ...
              : localeText(locale, `${selectedResource.units.length} 个单元`, `${selectedResource.units.length} units`)}
          </StatusPill>
        </div>
        ...
        {selectedResource.units.length === 0 ? (
          ...
          <div className="mt-5 grid gap-3 sm:grid-cols-2">
            {selectedResource.units.map((unit) => (
              <div key={unit.id} className="rounded-2xl border border-navy/10 bg-sand px-4 py-4">
